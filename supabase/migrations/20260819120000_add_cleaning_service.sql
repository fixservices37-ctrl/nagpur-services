-- =====================================================================
-- Add 'Cleaning' as an accepted service.
--
-- The list of accepted services is enforced in two places, both of which
-- need to be updated in lock-step with src/lib/services.ts:
--   1. The RLS insert policy on service_requests
--   2. The submit_service_request() RPC used by the public form
-- =====================================================================
...
CREATE POLICY "Anyone can submit a service request"
  ...
  WITH CHECK (
    ...
    AND service IN ('Electrical', 'Plumbing', 'Carpenter', 'RO / Water Filter', 'Cleaning')
...
BEGIN
  ...
  END IF;
  IF p_service NOT IN ('Electrical', 'Plumbing', 'Carpenter', 'RO / Water Filter', 'Cleaning') THEN
